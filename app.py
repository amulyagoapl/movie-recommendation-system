...
def recommend(movie_name):
    ...
    for i in movies_list:
        # Extracting the actual unique movie ID from your dataframe
        actual_movie_id = movies.iloc[i[0]].movie_id

        recommended_movies.append(movies.iloc[i[0]].title)
        # Passing the actual_movie_id to the API instead of the index loop tracker
        recommended_movies_poster.append(fetch_poster(actual_movie_id))

    return recommended_movies, recommended_movies_poster


# OPTIMIZATION: Compute similarity on the fly to bypass GitHub file limits and save Render RAM
@st.cache_resource
def load_and_compute_data():
    # Load your lightweight processed movie dictionary (Ensure movie_dict.pkl has 'tags' column)
    movies_dict = pickle.load(open('movie_dict.pkl', 'rb'))
    movies_df = pd.DataFrame(movies_dict)

    # Recreate the text vectors and calculate similarity on boot-up
...
movies, similarity = load_and_compute_data()
...
if st.button('Recommend'):
    with st.spinner('Fetching recommendations...'):
        ...
        # Loop rendering to map images to correct columns
        for index in range(5):
            with columns[index]:
                st.text(names[index])
                st.image(posters[index])
